...
import { Plus, Pencil, Trash2, LogOut, FileText, ChevronLeft } from "lucide-react";

export const Route = createFileRoute("/predictions/admin")({
  head: () => ({
    meta: [
      { title: "Управление прогнозами — NEUROMEIN" },
...
function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (!isAdmin) return;
    load();
  }, [loading, session, isAdmin, navigate, load]);

  if (loading) {
    return (
      <Layout>
        <div className="max-w-[1100px] mx-auto pt-12 px-4 text-text-secondary text-[14px]">
          Загрузка…
        </div>
      </Layout>
    );
  ...
  if (!isAdmin) {
    return (
      <Layout>
        <div className="max-w-[600px] mx-auto pt-16 px-4 text-center">
          <h1 className="text-[24px] font-medium text-text-primary mb-3">
            Доступ закрыт
          </h1>
  ...
  return (
    <Layout>
      <div className="max-w-[1100px] mx-auto pt-4 pb-24 px-4 sm:px-6">
        <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
          <div>
            <Link
              to="/predictions"
              className="inline-flex items-center gap-1 text-[12px] text-text-tertiary hover:text-text-secondary mb-2"
            >
              <ChevronLeft size={14} /> На страницу прогнозов
            </Link>
            <h1 className="text-[28px] font-medium tracking-[-0.02em] text-text-primary">
              Управление прогнозами
            </h1>
            <p className="text-[13px] text-text-secondary mt-1">
              {session.user.email} · {items.length} {items.length === 1 ? "запись" : "записей"}
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => signOut()}
              className="inline-flex items-center gap-2 h-10 px-4 rounded-lg bg-bg-card text-text-secondary text-[14px] hover:text-text-primary"
              title="Выйти"
            >
              <LogOut size={14} />
            </button>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field label="Источник (работа)">
            <select
              ...
              onChange={(e) => update("source_work", e.target.value)}
              className={inputCls}
            >
              {SOURCE_WORKS.map((s) => (
          ...
          <Field label="Название работы">
            <input
              required
              value={form.source_work_title}
              onChange={(e) => update("source_work_title", e.target.value)}
              className={inputCls}
            />
          </Field>
          <Field label="Раздел (опц.)">
            <input
              ...
              onChange={(e) => update("source_section", e.target.value)}
              placeholder="гл. 2.1"
              className={inputCls}
            />
          </Field>
          <Field label="Страница (опц.)">
            <input
              ...
              onChange={(e) => update("source_page", e.target.value)}
              className={inputCls}
            />
          </Field>
          <Field label="Дата прогноза">
            <input
              ...
              onChange={(e) => update("date_made", e.target.value)}
              className={inputCls}
            />
          </Field>
          <Field label="Горизонт">
            <input
              ...
              placeholder="Q1 2026 / Конец 2026 / К 2028"
              className={inputCls}
            />
          </Field>
          <Field label="Статус">
          ...
          <Field label="Дата обновления статуса">
            <input
              type="date"
              required
              value={form.status_updated}
              onChange={(e) => update("status_updated", e.target.value)}
              className={inputCls}
            />
          </Field>
        ...
        <div className="flex items-center justify-end gap-2 pt-3 border-t border-border">
          <button
            type="button"
            onClick={onClose}
            className="h-10 px-4 rounded-lg text-[14px] text-text-secondary hover:text-text-primary"
          >
            Отмена
          </button>
          <button
            type="submit"
            disabled={saving}
            className="h-10 px-5 rounded-lg bg-text-primary text-bg text-[14px] font-medium hover:opacity-90 disabled:opacity-50"
          >
      ...
      {loading ? (
        ...
        <ul className="flex flex-col gap-2">
          {items.map((ev) => (
            <li
              key={ev.id}
              className="flex items-start gap-3 bg-bg-deep rounded-lg p-3"
            >
              <span className="text-[11px] text-text-tertiary tabular-nums shrink-0 mt-0.5">
                {ev.evidence_date}
              </span>
              <div className="flex-1 min-w-0">
...
function Modal({
  ...
  return (
    ...
    >
      <div
        className="bg-bg border border-border rounded-2xl w-full max-w-[720px] max-h-[90vh] flex flex-col overflow-hidden"
